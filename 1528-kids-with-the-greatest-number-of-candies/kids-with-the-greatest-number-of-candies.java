class Solution {
    public List<Boolean> kidsWithCandies(int[] candies, int extraCandies) {
        ...
        for (int kidIndex = 0; kidIndex < candies.length; kidIndex++) {
            if (maximumCandies < candies[kidIndex]) {
                maximumCandies = candies[kidIndex];
            }
        }
        for (int kidIndex = 0; kidIndex < candies.length; kidIndex++) {
            if (maximumCandies <= candies[kidIndex] + extraCandies) {
                result.add(true);
            }
            else{
                result.add(false);
